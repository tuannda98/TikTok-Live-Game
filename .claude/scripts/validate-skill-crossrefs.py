...
Scans SKILL.md bodies for /apw: references, builds a directed graph,
...
# Match /apw:skill-name but NOT inside code fences
CODE_FENCE_RE = re.compile(r"```[\s\S]*?```", re.MULTILINE)
SKILL_REF_RE = re.compile(r"/apw:([a-z0-9][\w-]*)")
...
def _extract_body_refs(content: str, own_name: str) -> set[str]:
    """Extract /apw: skill references from body text, excluding code fences."""
    m = FRONTMATTER_RE.match(content)
    body = content[m.end():] if m else content
    # Remove code fences to avoid false positives
    body = CODE_FENCE_RE.sub("", body)
    refs = set(SKILL_REF_RE.findall(body))
    # Normalize: strip apw: prefix if present in the captured group
    # The regex captures what's after /apw: so "plan" from "/apw:plan"
    refs.discard(own_name)
    # Also discard dir-name form of own name (e.g. "ck-plan" vs "plan")
    refs.discard(own_name.replace("apw:", ""))
    return refs
...
def _run_self_tests():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fixtures = {
            "cook": '---\nname: apw:cook\ndescription: "Cook things"\ncategory: utilities\nkeywords: [impl]\n---\n# Cook\nAfter planning, run /apw:scout then /apw:code-review.\n',
            "plan": '---\nname: apw:plan\ndescription: "Plan things"\ncategory: utilities\nkeywords: [plan]\n---\n# Plan\nThis skill creates plans.\n',
            "scout": '---\nname: apw:scout\ndescription: "Scout"\ncategory: dev-tools\nkeywords: [scout]\nrequires: [apw:scout]\n---\n# Scout\nExplore code. See /apw:cook for next step.\n```\n/apw:plan should not match inside fence\n```\n',
            "orphan-skill": '---\nname: apw:orphan\ndescription: "Orphan"\ncategory: other\nkeywords: []\n---\n# Orphan\nNo refs here.\n',
            "broken-ref": '---\nname: apw:broken\ndescription: "Broken"\ncategory: other\nkeywords: []\n---\n# Broken\nSee /apw:nonexistent for help.\nAlso /apw:cook is good.\n',
        }
        ...
        print("Self-tests:")
        # T1: Detects /apw:scout in cook's body
        _assert("scout" in graph["edges"].get("cook", []), "T1: Detects /apw:scout ref in body")
        # T2: cook→code-review edge exists
        _assert("code-review" in skills["cook"]["body_refs"] or "code-review" in graph["edges"].get("cook", []),
                "T2: Detects /apw:code-review ref in body")
        ...
        _assert(len(broken_refs) > 0, "T6: Broken reference /apw:nonexistent detected")
        ...
        # T8: Code fence exclusion (scout has /apw:plan in code fence — should NOT count)
