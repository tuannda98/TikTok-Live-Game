...
Trace bugs backward through call stack to find original trigger. Fix at source, not symptom. Includes `scripts/find-polluter.sh` for bisecting test pollution (Unix/macOS only — on Windows, run from WSL or Git Bash).
...
Visual verification of frontend implementations via Chrome MCP / `chrome-devtools-mcp` or `agent-browser`. Detect if frontend-related → check browser tool availability → screenshot + console error check → report. Skip if not frontend.
...
- **Codebase:** `apw:docs-seeker` skill for package/plugin docs; `apw:repomix` skill for codebase summary
- **Scouting:** `/apw:scout` or `/apw:scout ext` for finding relevant files
- **Frontend:** Chrome MCP / `chrome-devtools-mcp`, `agent-browser`, or project-native browser tests for visual verification
...
**Typically follows:** `/apw:scout` (after locating relevant code)
**Typically precedes:** `/apw:fix-bug` (fix the diagnosed issue), `/apw:brainstorm` (explore solutions for complex problems)
**Related:** `/apw:scout` (discover before debugging), `/apw:fix-bug` (fix after diagnosing)
