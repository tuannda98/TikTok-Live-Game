...
**Typically precedes:** `/apw:debug` (debug after scouting), `/apw:fix-bug` (fix after locating code), `/apw:code-review` (scout edge cases before review)
**Related:** `/apw:debug` (investigate after scouting), `/apw:brainstorm` (explore after scouting)
