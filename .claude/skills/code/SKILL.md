...
# Code - Smart Feature Implementation
...
**Example:**
```
/apw:code "Add user authentication to the app" --fast
/apw:code path/to/plan.md --auto
...
User override: If user invoked `--no-test`, item 2 is downgraded to a warning. Surface the unverified-tests risk in the finalize `AskUserQuestion` so the user accepts the trade-off rather than having it silently chosen. Items 1, 3, 4, 5 remain enforceable via the mandatory `apw:code-reviewer` subagent.
...
- **Code Review (MANDATORY):** Spawn `apw:code-reviewer` subagent with explicit checks:
...
- **Finalize (MANDATORY - never skip):**
  1. **Activate `/apw:project-management` skill (MANDATORY)** → run full plan sync-back across ALL `phase-XX-*.md` (not only current phase), update `plan.md` status/progress, hydrate Claude Tasks, generate progress report
  2. `apw:docs-manager` subagent → update `./docs` if changes warrant
  3. `TaskUpdate` → mark all Claude Tasks complete after sync-back verification (skip if Task tools unavailable)
  4. Ask user if they want to commit via `apw:git-manager` subagent
  5. Run `/apw:journal` to write a concise technical journal entry upon completion
...
| Finalize | `/apw:project-management` skill + `docs-manager`, `git-manager` subagents | **MUST** invoke all |
...
**Typically follows:** `/apw:plan` (execute a plan), `/apw:brainstorm` (implement agreed solution)
**Typically precedes:** `/apw:code-review` (review after implementation), `/apw:test` (validate changes)
**Related:** `/apw:fix-bug` (alternative for bug fixes), `/apw:plan` (create plan before coding)
