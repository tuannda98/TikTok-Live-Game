...
- Use `/apw:scout ext` or `scout` agent for codebase search
...
- Use `apw:planner` agent with research context
- Create `plan.md` + `phase-XX-*.md` files

**Fast:**
- Use `/apw:plan --fast` with scout results only
- Minimal planning, focus on action

**Parallel:**
- Use `/apw:plan --parallel` for dependency graph + file ownership matrix
...
- Use `AskUserQuestion` to ask: "Validate the plan or approve plan to start implementation?" - "Validate" / "Approve" / "Abort" / "Other" ("Request revisions")
  - "Validate": run `/apw:plan validate` skill invocation
...
- **MUST** spawn `apw:tester` subagent: `Task(subagent_type="apw:tester", prompt="Run test suite", description="Run tests")`
- If failures: **MUST** spawn `apw:debugger` subagent → fix → repeat
...
- **MUST** spawn `apw:code-reviewer` subagent with explicit (a-e) checks and scout/acceptance context:
  ```
...
1. **MUST** activate `/apw:project-management` skill (MANDATORY) — run full sync-back for [plan-path]: reconcile all completed Claude Tasks with all phase files, backfill stale completed checkboxes across every phase, then update plan.md frontmatter/table progress. Do NOT only mark current phase.
2. **MUST** spawn in parallel:
   - `Task(subagent_type="apw:docs-manager", prompt="Update docs for changes.", description="Update docs")`
...
6. **MUST** spawn git subagent: `Task(subagent_type="apw:git-manager", prompt="Stage and commit changes", description="Commit")`

**CRITICAL:** Step 6 is INCOMPLETE without activating `/apw:project-management` skill AND spawning `docs-manager` + `git-manager` subagents. DO NOT skip.
...
- **MANDATORY DELEGATION:** Steps 4, 5, 6 MUST delegate via Task tool / skill activation. DO NOT implement directly.
  ...
  - Step 6: `/apw:project-management` skill, `docs-manager`, `git-manager`
