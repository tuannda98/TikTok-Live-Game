...
2. Activate `apw:debug` skill. Use `apw:debugger` subagent if needed.
...
- Ask to commit via `apw:git-manager` subagent
- Run `/apw:journal`
...
| 2 | `debugging`, `sequential-thinking`, `apw:debugger` subagent, parallel `Explore`, (`problem-solving` auto) |
| 3 | `problem-solving` (if stuck), `sequential-thinking` (complex logic) |
| 4 | `apw:tester` subagent, parallel `Bash` verification |
| 5 | `apw:code-reviewer` subagent |
