...
Activate `apw:debug` skill. Activate `sequential-thinking` for structured analysis.
...
Use `apw:code-reviewer` subagent for quick review with explicit side-effect sweep.
...
1. **Activate `/apw:project-management` skill (MANDATORY)** → sync plan/task status if fix is part of a plan, update progress, hydrate Claude Tasks.
2. Spawn `apw:docs-manager` subagent if API/behavior changed.
3. `TaskUpdate` to mark Claude Tasks complete.
4. Spawn `apw:git-manager` subagent to commit.
5. Run `/apw:journal` to log decisions.
...
| 4 | `apw:code-reviewer` subagent |
| 5 | Report |
| 6 | `/apw:project-management` (MANDATORY), `docs-manager`, `git-manager`, `/apw:journal` |
...
- Step 6 finalize is MANDATORY for every fix — `/apw:project-management` is NOT optional
