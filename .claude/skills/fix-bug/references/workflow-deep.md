...
**Mandatory:** Activate `apw:scout` skill or launch 2-3 `Explore` subagents in parallel:
```
...
2. Activate `apw:debug` skill (systematic-debugging + root-cause-tracing).
...
Use `apw:researcher` subagent for external knowledge.
...
Use `apw:planner` subagent to create implementation plan.
...
Use `apw:code-reviewer` subagent.
...
- Use `apw:docs-manager` subagent for documentation
- Use `apw:git-manager` subagent for commit
- Run `/apw:journal`
...
**Frontend:** Use `agent-browser`, Chrome MCP / `chrome-devtools-mcp`, or any relevant project-native browser tests to verify.
