...
| `/apw:project-management` | Step 6 | MANDATORY for sync-back and progress tracking, every fix |
...
|----------|------------------|
| Quick | `scout` (minimal), `debugging`, `sequential-thinking`, `code-review`, `/apw:project-management`, parallel `Bash` verification |
...
| Step 6: Finalize | Report → `/apw:project-management` (MANDATORY) → `docs-manager` → `TaskUpdate` → `git-manager` → `/apw:journal` |
...
| "latest docs", "best practice" | `apw:researcher` subagent |
