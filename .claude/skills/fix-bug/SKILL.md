...
1. Activate `apw:scout` skill OR launch 2-3 parallel `Explore` subagents
...
2. Activate `apw:debug` skill (systematic-debugging + root-cause-tracing techniques).
3. Activate `apw:sequential-thinking` skill — form hypotheses through structured reasoning, NOT guessing.
...
4. **Code review (delegate):** Spawn `apw:code-reviewer` subagent with explicit instructions to check: (a) root cause actually addressed (not symptom-patched), (b) no broken business logic in blast radius, (c) no new failure modes, (d) follows existing patterns from scout. Pass scout summary + diagnosis report as context.
...
2. **Activate `/apw:project-management` skill (MANDATORY)** → sync plan/task status (if fix is part of a plan), update progress, hydrate Claude Tasks, generate status report
3. `apw:docs-manager` subagent → update `./docs` if changes warrant (NON-OPTIONAL)
4. `TaskUpdate` → mark ALL Claude Tasks `completed` (skip if Task tools unavailable)
5. Ask user if they want to commit via `apw:git-manager` subagent
6. Run `/apw:journal` to write a concise technical journal entry upon completion
...
**Typically follows:** `/apw:debug` (after root cause analysis), `/apw:scout` (after locating affected code)
**Typically precedes:** `/apw:code-review` (review the fix), `/apw:test` (validate the fix)
**Related:** `/apw:code` (alternative for feature work), `/apw:debug` (diagnose before fixing)
