...
Browser-based visual testing via `agent-browser`, `web-testing`, or project-native Playwright/Vitest/k6 commands. Covers screenshots, responsive checks, accessibility audits, form automation, and console error collection.
...
6. For frontend: run UI tests via `agent-browser`, `web-testing`, or project-native browser tests
...
- **Browser**: `agent-browser` for live browser interaction; `web-testing` or project-native Playwright/Vitest/k6 for repeatable UI tests
- **Analysis**: `apw:ai-multimodal` skill for screenshot analysis
- **Debugging**: `apw:debug` skill when tests reveal bugs requiring investigation
- **Thinking**: `apw:sequential-thinking` skill for complex test failure analysis
...
**Typically follows:** `/apw:code` (test after implementation), `/apw:fix-bug` (test after bug fix)
**Typically precedes:** `/apw:code-review` (review after tests pass)
**Related:** `/apw:code` (implement then test), `/apw:fix-bug` (fix then test)
