...
2. Use `apw:planner` subagent to create plan
3. Hydrate tasks (unless `--no-tasks`)
4. **Implementation option:** `/apw:code {absolute-plan-path}/plan.md`

**Why no default code automation?** Fast planning reduces planning overhead, but implementation still requires a user choice. Add `--auto` only when the user explicitly asks to skip code review gates.
...
2. Read codebase docs; if stale/missing: run `/apw:scout` to search codebase
3. Gather research + scout report filepaths → pass to `apw:planner` subagent
...
7. **Context reminder:** `/apw:code {absolute-plan-path}/plan.md`

**Why no code flag?** Thorough planning needs interactive review gates.
...
2. Read all relevant docs and use `/apw:scout` across the affected areas
...
8. Output the standard `/apw:code {absolute-plan-path}/plan.md` reminder
...
7. **Context reminder:** `/apw:code --parallel {absolute-plan-path}/plan.md`
...
**Invocation:** Run `/apw:plan red-team {plan-directory-path}`.
```
/apw:plan red-team {plan-directory-path}
```
...
| `auto` | Run `/apw:plan validate {plan-directory-path}` |
| `off` | Skip validation |

**Invocation (when prompt mode, user says yes):** Run:
```
/apw:plan validate {plan-directory-path}
```
...
| Mode | Code Command |
|------|-----------------------------|
| fast | `/apw:code {path}/plan.md` |
| hard | `/apw:code {path}/plan.md` |
| deep | `/apw:code {path}/plan.md` |
| parallel | `/apw:code --parallel {path}/plan.md` |
| two | `/apw:code {path}/plan.md` |

If planning ran with `--tdd`, append `--tdd` to the reminder above so code keeps
the tests-first execution path. Example:
`/apw:code {path}/plan.md --tdd`

> **Best Practice:** Run `/clear` before implementing to reduce planning-context carryover.
> Then, if the user chooses implementation, run the code command above.
