...
Claude Tasks are **ephemeral** — they die when the session ends. `~/.claude/tasks/` holds lock files only, NOT task data (on Windows, replace `~/` with `%USERPROFILE%\` (cmd) or `$HOME\` (PowerShell)). Plan files (plan.md, phase-XX.md with checkboxes) are the **persistent** layer.
...
| Plan will be executed by code | Yes | Seamless handoff |
...
## User-Approved Code Continuation

### Same-Session (user approves code in current session)

1. Planning hydrates tasks → tasks exist in session
2. Agent stops and asks the user which next step they want
3. If the user approves implementation, Code Step 3: `TaskList` → finds existing tasks → picks them up
4. Code skips re-creation and begins the approved implementation path

### Cross-Session (new session, resume plan)

1. User runs `/apw:code path/to/plan.md` in new session
2. Code Step 3: `TaskList` → empty (tasks died with old session)
3. Code reads plan files → re-hydrates from unchecked `[ ]` items
4. Already-checked `[x]` items = done, skip those

### Sync-Back (code Step 6)

1. `TaskUpdate` marks all session tasks complete.
2. `apw:project-manager` subagent runs full-plan sync-ba
