...
3. If no plan found → Ask user to specify path or run `/apw:plan` first
...
## Next Steps
Remind user they can run `/apw:plan validate {plan-directory-path}` before implementation.
When the user approves implementation, run `/apw:code {ABSOLUTE_PATH_TO_PLAN_DIR}/plan.md`.
