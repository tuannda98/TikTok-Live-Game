...
**Process:**
1. Invoke `/apw:scout` with edge-case-focused prompt
...
- **PR Review:** `/apw:code-review #123` → fetch diff → full 3-stage review on PR changes
- **Commit Review:** `/apw:code-review abc1234` → review specific commit with full pipeline

## Codebase Analysis Subcommands

| Subcommand | Reference | Purpose |
|------------|-----------|---------|
| `/apw:code-review codebase` | `references/codebase-scan-workflow.md` | Scan & analyze the codebase |
| `/apw:code-review codebase parallel` | `references/parallel-review-workflow.md` | Ultrathink edge cases, then parallel verify |
...
**Typically follows:** `/apw:code` (review after implementation), `/apw:fix-bug` (review after bug fix)
**Typically precedes:** `/apw:journal` (document after review passes)
**Related:** `/apw:scout` (scout before reviewing), `/apw:test` (test before reviewing)
