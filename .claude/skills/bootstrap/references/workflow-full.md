...
2. Use `apw:planner` + multiple `apw:researcher` subagents in parallel to find best-fit stack
...
2. Use `apw:ui-ux-designer` + `apw:researcher` subagents in parallel:
...
3. `apw:ui-ux-designer` creates:
   - Design guidelines at `./docs/design-guidelines.md`
   - Wireframes in HTML at `./docs/wireframe/`
4. If no logo provided: generate with `apw:ai-multimodal` skill
5. Screenshot wireframes with `apw:agent-browser` → save to `./docs/wireframes/`
...
Activate **apw:plan** skill: `/apw:plan --hard <requirements>`
...
Activate **apw:code** skill: `/apw:code <plan-path>` (interactive mode — review gates at each step)
