# Shared Phases (All Modes)

These phases apply after planning is complete and code skill is activated.
The `apw:code` skill handles most of these — this reference documents bootstrap-specific guidance.

## Implementation

Handled by **apw:code** skill. Bootstrap-specific notes:
- Use main agent to implement step by step per plan in `./plans`
- Use `apw:ui-ux-designer` subagent for frontend per `./docs/design-guidelines.md`
- Asset pipeline: `apw:ai-multimodal` (generate/analyze) → `imagemagick` (crop/resize) → background removal if needed
- Run type checking and compile after each phase

## Testing

Handled by **apw:code** skill. Bootstrap-specific notes:
- Write real tests — NO fake data, mocks, cheats, tricks, temporary solutions
- `apw:tester` subagent runs tests → report to main agent
- If failures: `apw:debugger` subagent → fix → repeat until all pass
- DO NOT ignore failed tests to pass build/CI

## Code Review

Handled by **apw:code** skill. Bootstrap-specific notes:
- `apw:code-reviewer` subagent reviews code
- If critical issues: fix → retest → repeat
- Report summary to user when all tests pass and code reviewed

## Documentation

After code review passes. Use `apw:docs-manager` subagent to create/update:
- `./docs/README.md` (≤300 lines)
- `./docs/codebase-summary.md`
- `./docs/project-overview-pdr.md` (Product Development Requirements)
- `./docs/code-standards.md`
- `./docs/system-architecture.md`

Use `apw:project-manager` subagent to create:
- `./docs/project-roadmap.md`
- `./docs/project-changelog.md`
- Update plan/phase status to complete

## Onboarding

Guide user to get started with the project:
- Ask 1 question at a time, wait for answer before next
- Example: instruct user to obtain API key → ask for key → add to env vars
- If user requests config changes, repeat until approved

## Final Report

1. Summary of all changes, brief explanations
2. Guide user to get started + suggest next steps
3. Ask user if they want to commit/push:
   - If yes: `apw:git-manager` subagent to commit (and push if requested)
   - `--fast` mode: keep the same commit approval gate; speed mode does not imply git automation

**Report rules:**
- Sacrifice grammar for concision
- List unresolved questions at end, if any
