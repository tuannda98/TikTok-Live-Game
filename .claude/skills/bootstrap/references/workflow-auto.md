...
Spawn multiple `apw:researcher` subagents in parallel:
...
1. Use `apw:planner` + multiple `apw:researcher` subagents in parallel for best-fit stack
...
1. Use `apw:ui-ux-designer` + `apw:researcher` subagents in parallel:
   - Research style, trends, fonts (predict Google Fonts name, NOT just Inter/Poppins), colors, spacing, positions
   - Describe assets for `apw:ai-multimodal` skill generation
2. `apw:ui-ux-designer` creates:
   - Design guidelines at `./docs/design-guidelines.md`
   - Wireframes in HTML at `./docs/wireframe/`
3. If no logo provided: generate with `apw:ai-multimodal` skill
4. Screenshot wireframes with `apw:agent-browser` → save to `./docs/wireframes/`
...
Activate **apw:plan** skill: `/apw:plan --auto <requirements>`
...
Activate **apw:code** skill: `/apw:code --auto <plan-path>`
- Skips code review gates because `--auto` was explicitly requested
