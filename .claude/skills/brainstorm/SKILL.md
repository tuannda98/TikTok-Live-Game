...
## Collaboration Tools
- Consult the `apw:planner` agent to research industry best practices and find proven solutions
- Engage the `apw:docs-manager` agent to understand existing project implementation and constraints
- Use `WebSearch` tool to find efficient approaches and learn from others' experiences
- Use `apw:docs-seeker` skill to read latest documentation of external plugins/packages
- Leverage `apw:ai-multimodal` skill to analyze visual materials and mockups
- Query `psql` command to understand current database structure and existing data
- Employ `apw:sequential-thinking` skill for complex problem-solving that requires structured analysis
...
1. **Scout Phase (MANDATORY FIRST STEP)**: Always run before anything else.
   - Use `apw:scout` skill (or Glob/Grep directly for small repos) to map files relevant to the user's topic
...
10. **Journal Phase**: Run `/apw:journal` to write a concise technical journal entry upon completion.
...
**Typically follows:** `/apw:debug` (brainstorm solutions for diagnosed issues), `/apw:scout` (brainstorm after discovery)
**Typically precedes:** `/plan` (plan the agreed solution)
**Related:** `/plan` (plan after brainstorming), `/apw:debug` (debug before brainstorming)
