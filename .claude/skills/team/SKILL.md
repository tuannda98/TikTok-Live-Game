...
# Agent Teams - Native Orchestration Engine
...
```
/apw:team <template> <context> [flags]
```
...
4. Do NOT fall back to subagents. `/apw:team` MUST use Agent Teams or abort.
...
## ON `/apw:team research <topic>` [--researchers N]:

*Wraps /apw:research skill -- scope, gather, analyze, report.*
...
11. **JOURNAL**: Run `/apw:journal` to write a concise technical journal entry upon completion

---

## ON `/apw:team cook <plan-path-or-description>` [--devs N]:

*Wraps /apw:code skill -- plan, code, test, review, finalize.*
...
7. **DOCS SYNC EVAL** (MANDATORY for cook -- from /apw:code finalize):
   ```
...
## ON `/apw:team review <scope>` [--reviewers N]:

*Wraps /apw:code-review skill -- scout, review, synthesize with evidence gates.*
...
## ON `/apw:team debug <issue>` [--debuggers N]:

*Wraps /apw:fix-bug skill -- root-cause-first, adversarial hypotheses, disprove to converge.*
