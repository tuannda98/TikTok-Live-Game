...
### View Mode
- `/apw:preview <file.md>` - View markdown file in novel-reader UI
- `/apw:preview <directory/>` - Browse directory contents
- `/apw:preview --stop` - Stop running server

### Generation Mode (Markdown)
- `/apw:preview --explain <topic>` - Generate visual explanation (ASCII + Mermaid + prose)
- `/apw:preview --slides <topic>` - Generate presentation slides (one concept per slide)
- `/apw:preview --diagram <topic>` - Generate focused diagram (ASCII + Mermaid)
- `/apw:preview --ascii <topic>` - Generate ASCII-only diagram (terminal-friendly)

### Generation Mode (HTML)
- `/apw:preview --html --explain <topic>` - Self-contained HTML explanation
- `/apw:preview --html --slides <topic>` - Magazine-quality HTML slide deck
- `/apw:preview --html --diagram <topic>` - HTML diagram with zoom controls
- `/apw:preview --html --diff [ref]` - Visual diff review
- `/apw:preview --html --plan-review [plan-file]` - Plan vs codebase comparison
...
| Flag without topic | Ask user: "Please provide a topic: `/apw:preview --explain <topic>`" |
...
| Server startup failure | Check if port in use, try `/apw:preview --stop` first |
